...
def quantum_model_train(train_X, train_Y, validation_X, validation_Y, test_X, test_Y, depth=1):
    ...
    # Initialise weights
    w = np.array(np.split(np.random.uniform(size=(18*depth,), low=-1, high=1), depth),
...
for i in range(n_iteration):
    ...
    loss = quantum_model_train(train_X, train_Y, validation_X, validation_Y, test_X, test_Y, depth=3)
...
#plt.title('QAOA (L=3)', fontsize=16)
plt.xlabel('Predicted (QAOA, L=3)', fontsize=14)#, labelpad=20)  # Adjust labelpad for spacing
...
# Save the plot
#plt.savefig('ConfMatrix(QAOA, L=3_1).png')
